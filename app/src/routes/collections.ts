...
export const collections = new Hono<PrincipalEnv>();

collections.use("*", withPrincipal, requireWrite);
...
collections.post("/", async (c) => {
  ...
  const result = await createCollection(c.get("principal").vaultId, {
    slug: body.slug,
...
collections.patch("/:collectionId", async (c) => {
  ...
  const result = await updateCollection(c.get("principal").vaultId, c.req.param("collectionId"), {
    name: typeof body.name === "string" ? body.name : undefined,
    rule: "rule" in body ? body.rule : undefined,
    sortKey: typeof body.sortKey === "string" ? body.sortKey : undefined,
  });
...
collections.delete("/:collectionId", async (c) => {
  const gone = await deleteCollection(c.get("principal").vaultId, c.req.param("collectionId"));
